...
Presentation-neutral leaf module (depends only on ``data.models``) so both the
UI and the AI context builder share one source of truth. Kept in one module so
the future i18n pass (v1.0) only touches this file and the ``tr()``-wrapped
literals.
"""
