...
- ✅ **Thème sombre personnalisable (2026-07-11)** : la personnalisation des couleurs de fond marche aussi en mode sombre. `LIGHT_BACKGROUNDS` (crème/blanc/pierre/sauge/ciel/poudre) + `DARK_BACKGROUNDS` (brun défaut/noir doux/forêt/nuit/prune), stockés séparément par thème (`mise_bg_light` / `mise_bg_dark`, migration douce de l'ancien `mise_bg`→light). `refreshBgVars()` applique le preset du thème courant, `renderBgRow()` redessine au changement de thème. Accent (`mise_accent`) partagé entre les deux thèmes. Bouton « Réinitialiser le thème ». Garde-fou contraste : seule la famille fond (paper/surface/cream/line/rowhover) est éditable, le texte reste géré par le thème (mesuré ≥4,5:1 sur tous les presets). Réglages → Apparence.
- ✅ **Accueil épuré (2026-07-11)** : raccourci « Planifier ma semaine » retiré de l'accueil (redondant avec l'onglet Planning `navWeek`, conservé) ; « Trouver une idée » — ampoule agrandie, sous-titre retiré.
